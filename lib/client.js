...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
      ...
      try {
        return fetch(CONFIG_URL, {
          ...
          body: JSON.stringify({
            ...
            sidebarOpacity: store.sidebarOpacity,
            composerOpacity: store.composerOpacity,
          }),
        }).then((resp) => {
    ...
    function SettingsPage(props) {
      ...
      const onToggle = (e) => { store.enabled = e.target.checked; update(themeService) }
      const onMode = (id) => { store.mode = id; update(themeService) }
      const onOpacity = (e) => { store.opacity = Number(e.target.value) / 100; update(themeService) }
      const onBlur = (e) => { store.blur = Number(e.target.value); update(themeService) }
      const onSaturation = (e) => { store.saturation = Number(e.target.value) / 100; update(themeService) }
      const onSidebarOpacity = (e) => { store.sidebarOpacity = Number(e.target.value) / 100; update(themeService) }
      const onComposerOpacity = (e) => { store.composerOpacity = Number(e.target.value) / 100; update(themeService) }
      ...
      const details = React.createElement('div', { className: 'bgimg-details' },
        ...
        React.createElement('div', { className: 'bgimg-card' },
          ...
          React.createElement('div', { className: 'bgimg-sub' },
            '模糊与饱和度作用于输入卡片等玻璃浮层；侧边栏只做透明玻璃（因设置弹层为固定定位渲染在侧栏内，加模糊会把弹层锁在侧栏）。')
        ),
        React.createElement('div', { className: 'bgimg-card' },
          ...
          React.createElement('div', { className: 'bgimg-sub' },
            '数值越低越透明、越沉浸；侧边栏仅在「沉浸式全屏」下生效，输入区两种模式均生效。')
        ),
        React.createElement('div', { className: 'bgimg-card' },
          ...
          s.image
            ? React.createElement('div', { className: 'bgimg-url-row' },
              React.createElement('button', { type: 'button', className: 'bgimg-btn', onClick: () => { store.image = ''; update(themeService) } }, '移除图片')
            )
            : null,
          ...
          React.createElement('div', { className: 'bgimg-presets' },
            PRESETS.map((p) => React.createElement('button', {
              ...
              title: p.label,
              onClick: () => { setError(''); store.image = p.css; update(themeService) }
            },
        ...
        React.createElement('button', {
          type: 'button',
          className: 'bgimg-btn',
          onClick: () => { setUrl(''); setError(''); store.enabled = false; store.mode = 'fullscreen'; store.opacity = 0.9; store.image = ''; store.blur = 0; store.saturation = 1; store.sidebarOpacity = 0.5; store.composerOpacity = 0.72; update(themeService) }
        }, '恢复默认设置'),
        React.createElement('div', { className: 'bgimg-sub' },
...
.cvtE3a_card,.Nqubda_panel,.Nqubda_header,.Nqubda_row,.cvtE3a_business,.gNWCoW_inspectButton,.o3BgMG_inspectButton,.iWrAna_inspectButton,.CY-8Ka_inspectButton,.Y0dWHa_panelImage,.Y0dWHa_toolCatalogDefinition,._7yHdaG_editor,.nLMEza_objectiveInput{background:var(--dsw-alias-bg-layer-2) !important}
...
/* 毛玻璃：侧栏内层不再重复上色，只让外层 sidebarCol 承担半透明，避免透明度叠加。
   注意：侧栏不施加 backdrop-filter —— 设置弹层（.VOzbGW_overlay 为 position:fixed;z-index:1000）渲染在侧栏内，
   若在侧栏/其内层加 backdrop-filter，会让侧栏成为 fixed 后代的包含块、把设置弹层锁在侧栏里。
   因此侧栏只做“透明玻璃”，模糊仅用于输入卡片（其内部无 fixed 弹层，安全）。 */
