...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    const SETTINGS_KEYS = ['enabled', 'mode', 'opacity', 'image', 'blur', 'saturation', 'sidebarOpacity', 'composerOpacity']
    ...
    function SettingsPage(props) {
      ...
      const onToggle = (e) => { store.enabled = e.target.checked; update(themeService, ['enabled']) }
      ...
      const details = React.createElement('div', { className: 'bgimg-details' },
        ...
        React.createElement('div', { className: 'bgimg-card' },
          ...
          s.image
            ? React.createElement('div', { className: 'bgimg-url-row' },
              React.createElement('button', { type: 'button', className: 'bgimg-btn', onClick: () => { store.image = ''; update(themeService, ['image']) } }, '移除图片')
            )
            : null,
          ...
          React.createElement('div', { className: 'bgimg-presets' },
            PRESETS.map((p) => React.createElement('button', {
              ...
              title: p.label,
              onClick: () => { setError(''); store.image = p.css; update(themeService, ['image']) }
            },
        ...
        React.createElement('button', {
          type: 'button',
          className: 'bgimg-btn',
          onClick: () => { setUrl(''); setError(''); store.enabled = false; store.mode = 'fullscreen'; store.opacity = 0.9; store.image = ''; store.blur = 0; store.saturation = 1; store.sidebarOpacity = 0.5; store.composerOpacity = 0.72; update(themeService, SETTINGS_KEYS) }
        }, '恢复默认设置'),
        React.createElement('div', { className: 'bgimg-sub' },
