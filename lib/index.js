...
// 与 SETTINGS_SCHEMA 字段一一对应：config 路由只接受这些键的增量更新。
    ...
    scoped.effect(() => scoped.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          // HEAD 仅用于可达性探测（本地图片失效提示），不把文件内容读进内存。
          res.writeHead(200, {
            'content-type': IMAGE_MIME[extname(authorized).toLowerCase()] ?? 'application/octet-stream',
            'cache-control': 'no-cache',
          })
          res.end(req.method === 'HEAD' ? undefined : await readFile(authorized))
        } catch {
          res.writeHead(404)
          res.end()
        }
    ...
    scoped.effect(() => scoped.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        if (req.method === 'POST') {
          ...
          try {
            // 浅合并：客户端只提交变更字段，未提供的键保留当前值，
            // 避免高频滑杆的全量覆盖抹掉其它标签页/客户端刚写入的设置。
            const incoming = JSON.parse(body)
            if (incoming === null || typeof incoming !== 'object' || Array.isArray(incoming)) {
              throw new Error('request body must be a JSON object')
            }
            const current = scoped.settings.get(SETTINGS_NAMESPACE)
            const section = typeof current === 'object' && current !== null ? { ...current } : {}
            for (const key of SETTINGS_KEYS) {
              if (Object.prototype.hasOwnProperty.call(incoming, key)) section[key] = incoming[key]
            }
