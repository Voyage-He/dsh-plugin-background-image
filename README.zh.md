...
[English](https://github.com/Voyage-He/dsh-plugin-background-image/blob/main/README.md) | **简体中文**

一个用于 DeepSeek Harness（DSH）的背景图片插件：为界面设置网络图片、本地图片或预设渐变背景，并支持图片不透明度、显示范围、毛玻璃模糊、分区不透明度与对话区偏移调节，营造更沉浸的界面效果。
...
5. 根据需要调整图片不透明度、显示范围、**毛玻璃**（背景模糊强度、玻璃颜色饱和度，作用于侧边栏与输入卡片）、**分区不透明度**（侧边栏、输入区）与**对话区偏移**。数值越低越透明、越沉浸；侧边栏在「沉浸式全屏」下呈现毛玻璃，输入区在两种模式下均生效。

## 配置存储

背景开关、显示范围、图片不透明度、毛玻璃、分区不透明度与对话区偏移由 DSH 设置系统统一持久化到配置目录（默认 `~/.dsh/settings.yaml`），形如：
...
2. **依赖 DSH 内部 hashed 类名**：连续背景、玻璃效果与对话区偏移依赖 `.pI_x6G_frame`、`.wSkVaW_root`、`.ydkMvW_root`、`.hHd-Xa_root`、`.pI_x6G_sidebarCol`、`.uV2eYG_card`、`.wSkVaW_composerSeat`、`.wSkVaW_viewArea`、`.wSkVaW_composerHero` 等内部类名；DSH 升级若改动这些类名，对应效果会失效或错位。
